...
document.querySelector("#cart-click").addEventListener("click", (e) => {
  e.preventDefault();
  ...
  document.querySelectorAll(".quick-add__submit").forEach(button => {
    button.addEventListener("click", async (e) => {
      ...
      try {
        ...
        row.querySelector(".variant-item__totals.small-hide span.price").textContent = "$" + data.line_price.toFixed(2)/100;
        row.querySelector(".variant-item__totals.large-up-hide span.price").textContent = "$" + data.line_price.toFixed(2)/100;
        refreshCart();
